...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    ...
    if (keycode == STICKY_LAYER_TOGGLE && record->event.pressed) {
        default_layer_set(default_layer_state ? 0 : (1 << get_highest_layer(layer_state|default_layer_state)));
    }
...
#define INDEX_FINGER_COLOR  DIMMED(RGB_GOLD)
...
#define RIGHT_THUMB_COLOR   DIMMED(RGB_BLUE)
#define EXTRA_LAYER_COLOR   DIMMED(RGB_PURPLE)
